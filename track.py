...
def court_direction_paint(paints):
    left_threshold = 0.7 * VIDEO_HEIGHT
    right_threshold = 0.4 * VIDEO_HEIGHT


def define_court(features):
    # check for paint corners:
    paint_points = features["paint"]
    if len(paint_points) < 4:
        return None
    elif len(paint_points) > 4:
        return None

    sorted_x_corners = sorted(paint_points, key=lambda x: x[0])
    ...
    return {
        "top_left": sorted_left_y_corners[0][0],
        "top_right": sorted_right_y_corners[0][0],
        "bottom_left": sorted_left_y_corners[1][0],
        "bottom_right": sorted_right_y_corners[1][0],
    }


def homography(labels, threshold=0):
    # paint = define_court([label[0] for label in labels["paint"]])
    paint = define_court(labels)
    print(paint)
    if not paint:
        return None
...
def track():
    ...
    while (
        ...
        image = cv2.resize(
            image, (VIDEO_WIDTH, int(image.shape[0] * (VIDEO_WIDTH / image.shape[1])))
        )
